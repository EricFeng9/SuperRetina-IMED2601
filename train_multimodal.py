...
from dataset.CF_251112_gen_proccessed.cf_dataset import CFRegistrationDataset
...
def validate(model, val_loader, device, epoch, save_dir, log_file, cached_transforms=None):
    """
    ...
    # 缓存变换矩阵，保证不同 Epoch 间验证集输入的随机扰动是一致的
    ...
    with torch.no_grad():
        for fixed_imgs, moving_imgs, kp_maps, input_with_labels, sample_ids in tqdm(val_loader, desc=f"Val Epoch {epoch}"):
            ...
            # 生成或加载一致的几何扰动
            if is_caching:
                # 按照计划：旋转 [-30, 30]，平移 [0.1, 0.2]
                affine_mov, grid_aff, grid_inv_aff = affine_images(moving_imgs, used_for='detector')
                cached_transforms[sample_id] = {
                    'grid_aff': grid_aff.cpu(),
                    'grid_inv_aff': grid_inv_aff.cpu()
                }
            else:
                if sample_id in cached_transforms:
                    grids = cached_transforms[sample_id]
                    grid_aff = grids['grid_aff'].to(device)
                    import torch.nn.functional as F
                    affine_mov = F.grid_sample(moving_imgs, grid_aff, align_corners=True)
                else:
                    affine_mov, grid_aff, grid_inv_aff = affine_images(moving_imgs, used_for='detector')
            
            # 提取跨模态特征
            det_fix, desc_fix = model.network(fixed_imgs)
            det_mov, desc_mov = model.network(affine_mov)
            
            for b in range(fixed_imgs.shape[0]):
                sample_id = sample_ids[b]
                
                # 有效区域屏蔽，防止边缘伪影干扰关键点提取
                valid_mask = (affine_mov[b:b+1] > 0.05).float()
                import torch.nn.functional as F
                valid_mask = -F.max_pool2d(-valid_mask, kernel_size=5, stride=1, padding=2)
                det_mov_masked = det_mov[b:b+1] * valid_mask
                
                # 提取关键点
                kps_fix = nms(det_fix[b:b+1], nms_thresh=0.005, nms_size=5)[0]
                kps_mov = nms(det_mov_masked, nms_thresh=0.005, nms_size=5)[0]
                ...
                if len(good) >= 4:
                    ...
                    # 利用 RANSAC 求解单应性矩阵，实现大尺度配准
                    M, mask = cv2.findHomography(dst_pts, src_pts, cv2.RANSAC, 5.0)
                    
                    if M is not None:
                        img_warped_np = affine_mov[b, 0].cpu().numpy() * 255.0
                        h, w = img_warped_np.shape
                        img_reg = cv2.warpPerspective(img_warped_np, M, (w, h))
                        
                        # 计算与 Ground Truth (原始对齐图像) 的均方误差
                        img_gt_np = moving_imgs[b, 0].cpu().numpy() * 255.0
                ...
                fix_np = (fixed_imgs[b, 0].cpu().numpy() * 255).astype(np.uint8)
                mov_in_np = (moving_imgs[b, 0].cpu().numpy() * 255).astype(np.uint8)
                mov_aff_np = (affine_mov[b, 0].cpu().numpy() * 255).astype(np.uint8)
...
def train_multimodal():
    ...
    parser.add_argument('--mode', '-m', type=str, choices=['cffa', 'cfoct'], help='Registration mode (cffa or cfoct)', default=None)
    ...
    train_set = CFRegistrationDataset(root_dir=root_dir, split='train', registration_type=reg_type)
    val_set_full = CFRegistrationDataset(root_dir=root_dir, split='val', registration_type=reg_type)
    
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=4)
    val_loader = DataLoader(val_set_full, batch_size=1, shuffle=False, num_workers=4)
    ...
    _, val_transforms_cache = validate(model, val_loader, device, 0, save_root, log_file, cached_transforms=None)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f'Epoch {epoch}/{num_epochs} | PKE_learn: {model.PKE_learn}')
        model.train()
        ...
        for fixed_imgs, moving_imgs, kp_maps, input_with_labels, names in tqdm(train_loader):
            fixed_imgs = fixed_imgs.to(device)
            moving_imgs = moving_imgs.to(device)
            kp_maps = kp_maps.to(device)
            ...
            value_maps = value_map_load(value_map_save_dir, names, input_with_labels, 
                                      fixed_imgs.shape[-2:], value_maps_running)
            ...
            with torch.set_grad_enabled(True):
                ...
                loss, number_pts, loss_det_item, loss_desc_item, enhanced_kp, enhanced_label, det_pred, n_det, n_desc = \
                    model(fixed_imgs, moving_imgs, kp_maps, value_maps, learn_index)
            ...
            total_samples += fixed_imgs.size(0)
        ...
        if epoch % 5 == 0:
            avg_mse, _ = validate(model, val_loader, device, epoch, save_root, log_file, cached_transforms=val_transforms_cache)
