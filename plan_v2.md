...
在原有的**几何校验**和**内容校验**（双重匹配）基础上，通过**描述子先行策略**增强特征学习：

1. **几何校验**：通过 $\mathcal{H}^{-1}$ 反投影寻找重现点 $S$。
2. **内容校验**：进行 Lowe's Ratio Test。
3. **动态标签更新**：$Y_t = Y_0 \cup \hat{S}$

**作用**：通过严格的几何和内容双重校验，自动发现两模态间可靠的匹配点。

---

## 4. 训练策略：四阶段课程学习（Curriculum Learning）

引入**描述子热身期**，解决"鸡生蛋"问题，并采用**渐衰权重**调节掩码的影响力。

| 阶段 | 周期 (Epochs) | 掩码权重最小值 | 状态 | 目的 |
|------|---------------|----------------|------|------|
| **0. 描述子热身期** | 0 - 20 | N/A | **仅描述子训练** | 冻结检测器，利用 GT 关键点强制训练描述子，解决 Ratio Test 失败问题 |
| **1. 全局感知期** | 20 - 50 | min_weight = 0.3 | PKE 开启 | 让模型利用全局信息（含背景）学习初步的 ±30° 几何对齐 |
| **2. 血管聚焦期** | 50 - 80 | min_weight = 0.1 | PKE 开启 | **强化血管引导**。PKE 进入活跃期，模型细化血管特征 |
| **3. 泛化增强期** | 80 - 100 | min_weight = 0.5 | PKE 开启 | 减弱掩码对比度，模拟弱掩码环境 |
| **4. 完全泛化期** | 100+ | min_weight = 1.0 | PKE 开启 | **掩码权重拉平**。模型完全依赖学到的特征进行配准，背景和血管同等重要 |
