...
def test_on_real():
    ...
    for i in tqdm(range(len(test_set)), desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # 为了统一评估，将 moving 图像 resize 到和 fix 相同尺寸 (与 train_multimodal_v3_3.py 对齐)
            if (h_m, w_m) != (h_f, w_f):
            ...
            # Evaluation with measurement.py (使用统一尺寸后的 moving 图像)
            ...
            # Save raw fix and moving images (moving 使用统一尺寸后的版本)
