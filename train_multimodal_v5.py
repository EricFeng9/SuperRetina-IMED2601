"""
基于v4.1, 升级为v5版本:
1. 引入 Vessel-Guided Loss (血管引导加权):
   - 利用 GT Vessel Mask 对描述子 Loss 进行加权
   - 血管区域权重高，背景区域权重低
2. 采用三阶段课程学习 (Curriculum Learning):
   - Phase 1 (Teaching): Vessel Weight = 10.0 (强关注)
   - Phase 2 (Weaning): Vessel Weight 10.0 -> 1.0 (线性衰减)
   - Phase 3 (Independence): Vessel Weight = 1.0 (自由探索)
3. 保留 v4.1 的 Shared Encoder + Inversion + Supervised PKE 策略
"""
...
def calculate_vessel_weight(epoch):
    """
    计算当前 Epoch 的血管加权权重 (v5.1 Corrected Schedule)
    Phase 1 (1-20): Supervised GT. All pts are vessels. Weight irrelevant (1.0).
    Phase 2 (21+): PKE Open (with Noise). High Weight (5.0) to focus on vessels.
    """
    if epoch <= 20: 
...
def train_multimodal():
    ...
    for epoch in range(1, num_epochs + 1):
        vessel_weight = calculate_vessel_weight(epoch)

        if epoch <= 20:
            phase = 3 # Enable PKE logic
            pke_supervised = True
            model.PKE_learn = True
            phase_name = f"Phase 1: Supervised PKE (GT Injection) (Weight={vessel_weight:.1f})"
        else:
            # Phase 2: Vessel Guided PKE (High Weight, No Decay)
            phase = 3 # Joint PKE
            pke_supervised = False
            model.PKE_learn = True
            phase_name = f"Phase 2: Vessel Guided PKE (Weight={vessel_weight:.1f})"
            ...
            with torch.set_grad_enabled(True):
                ...
                loss, number_pts, loss_det_item, loss_desc_item, enhanced_kp, enhanced_label, det_pred, n_det, n_desc = \
                    model(img0_input, img1, vessel_keypoints, value_maps, learn_index,
                          ...
                          pke_supervised=pke_supervised, vessel_weight=vessel_weight)
